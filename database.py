...
def init_db():
    ...
    if db_session.query(models.Team).count() == 0:
        Bucks = models.Team("Bucks", 5)
        db_session.add(Bucks)
        Celtics = models.Team("Celtics", 5)
        db_session.add(Celtics)
        Hawks = models.Team("Hawks", 2)
        db_session.add(Hawks)
        Lakers = models.Team("Lakers", 3)
        db_session.add(Lakers)
        Nuggets = models.Team("Nuggets", 4)
        db_session.add(Nuggets)
        Pacers = models.Team("Pacers", 2)
        db_session.add(Pacers)
        Pistons = models.Team("Pistons", 1)
        db_session.add(Pistons)
        Raptors = models.Team("Raptors", 2)
        db_session.add(Raptors)
        Sixers = models.Team("Sixers", 4)
        db_session.add(Sixers)
        Timberwolves = models.Team("Timberwolves", 2)
        db_session.add(Timberwolves)
        Warriors = models.Team("Warriors", 5)
        db_session.add(Warriors)
        Wizards = models.Team("Wizards", 1)
        db_session.add(Wizards)
        db_session.commit()
